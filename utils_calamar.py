...
def calcular_resultado(datos, pn_salida_total):
    ...
    for fila in datos:
        ...
        gramos_producto = config["tabla_1"].get(especie, {}).get(formato, 0)
        gramos_lamina = config["plastico_laminas"].get(formato, 0)
        ...
        precio_3 = config["tabla_3"].get(especie, {}).get(formato, 0)
        lamina_data = config["tabla_4"].get(lamina, {})
        base = lamina_data.get("base", 0)
        ajuste = lamina_data.get("ajustes", {}).get(formato, 0)
